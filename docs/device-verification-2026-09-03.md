...
**固件功能基线：** `a57ac8e`（架构迁移：SM 屏为唯一屏幕宿主、RenderPlan 唯一刷新来源、
...
旧版本号则只对该旧基线成立。新功能基线（a57ac8e）必须在重刷后才可把「最终版本通过」
...
- ❌ NOTE4 Alarms 页面复测（同一 ELF `v0.5.0-161-g0148ed9`）发现：ADD ALARM 完成 minute 确认后新 alarm 已加入列表但仍停留在 picker；添加多条 alarm 后连续 DOWN 停在 `+ ADD ALARM`，且长列表中 ADD 行不可见。`a57ac8e` 已修复完成后回列表选中新项、persist 失败回滚刷新、列表首尾循环、pending append 期间的编辑串行化，以及共享列表窗口滚动；新 ELF 尚未刷入复测。
