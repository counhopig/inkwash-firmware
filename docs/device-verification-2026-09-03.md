...
- ❌ `f7ea0ed` 真机复测：Wi‑Fi deinit 后 BLE preflight 已升至 `internal free=99475 largest=31744`，但随即在 `esp_task_stack_is_sane_cache_disabled()` 触发 assert；证据 `/tmp/inkwash-ble-wifi-deinit-f7ea0ed.log`。根因是 BLE worker 栈位于 PSRAM；当前候选改为 8 KiB internal pthread 栈，保留 Wi‑Fi 完整 deinit 及原 heap 门槛，待重新刷写。
- ⚠️ 当前候选修复（待真机）：BLE worker 使用 8 KiB internal pthread 栈（controller cache-disabled init 要求）；进入配对前由 sync task 独占的 `WifiManager` 丢弃（并由 `EspWifi` Drop 完整 deinit）Wi‑Fi driver，释放运行时缓冲，BLE worker 收到 SuspendForBle 完成后才初始化 NimBLE；Stop 完成后由同一 owner 重建全新 Wi‑Fi driver。即使 Wi‑Fi 从未 start，已构造的 driver 也会在 hand-off 中释放；启动前按 controller 所需的 internal+DMA 能力检查 heap，总空闲量门槛为 60 KiB、最大连续块门槛为 24 KiB，接纳 deinit 后实测的 `99475/31744` bytes；不足、Wi‑Fi busy 或同步任务失败均回 Settings。需按 §4 重建、刷写当前 ELF 后复测，确认不出现 `Malloc failed`/`emi.c 164`/`esp_task_stack_is_sane_cache_disabled`/WDT，且手机仍能发现并连接 `Inkwash`；退出配对后再验证 Sync Now。
...
| Todos | 列表 | 短 ENTER 切换完成；长 ENTER 回 Home | 长按 UP/DOWN 开 drawer，选择目标页 |
...
- BLE 启动失败或用户退出时，期望先完成 BLE Stop，再恢复 Wi-Fi；随后 Sync Now 应可再次连接并完成。
