...
- ⚠️ 当前候选修复（待真机）：BLE worker 使用 8 KiB PSRAM pthread 栈；启动前按 controller 所需的 internal+DMA 能力检查 heap，总空闲量门槛为 60 KiB、最大连续块门槛为 24 KiB。该门槛保留了相对旧失败请求 `0x1000` 的 6 倍连续块余量，同时接纳候选 ELF 实测的 `64831/31744` bytes；不足时回送失败并返回 Settings。ESP32-S3 controller 配置收紧为单连接、2 个 BLE activity、关闭 scan/DTM，并将 controller 代码放入 flash。需按 §4 重建、刷写当前 ELF 后复测，确认不再出现 `Malloc failed`/`emi.c 164`/WDT，且手机仍能发现并连接 `Inkwash`。
